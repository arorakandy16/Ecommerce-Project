...
public interface ProductRepository extends CrudRepository<Product,Long> {
    ...
    List<Product> findAllByCategoryIdForCustomerAdmin(@Param("categoryId") Long categoryId, Pageable pageable);

    @Transactional
    @Modifying
    @Query(value = "update Product set is_active=:is_active where productId=:productId")
    void isActivateProduct(@Param("productId") Long productId,@Param("is_active") boolean is_active);

    @Transactional
    @Modifying
    @Query(value = "update Product set is_deleted = true where productId=:id")
