...
public class ProductVariationService {
    ...
    public String addProductVariation(ProductVariationDto productVariationDto) {
        ...
        Optional<ProductCategory> category = productCategoryRepository.findById(product1.getProductcategory().getPcId());
        ...
        for (Map.Entry<String, String> fieldValues : metaData.entrySet()) {
            ...
            CategoryMetadataFieldValues values = metadataFieldValuesRepository.findMetadataFieldValue(categoryMetadataField.getId(),pcId);
    ...
    public ProductVariant findProductVariation(Long variationId) {
        Optional<ProductVariant> productVariation = productVariationRepository.findById(variationId);
        Product product = productVariation.get().getProduct();
        if(product.isIs_deleted())
            throw new ProductNotFoundException("Product does not exist");
        Seller seller = product.getSeller();
        if (seller.getEmail().equals(getLoggedInSeller())) {
    ...
    public String updateProductVariation(Long id, UpdateVariationDto updateVariationDto) {
        Optional<ProductVariant> productVariationOptional = productVariationRepository.findById(id);
        ...
        if (productVariation.getProduct().getSeller().getUserid() != getLoggedInSeller().getUserid()) {
        ...
        if (updateVariationDto.getProductVariationImage()!= null) {
            productVariation.setProductVariationImage(updateVariationDto.getProductVariationImage());
        }
        ...
        if (updateVariationDto.getMetadata() != null) {
            ...
            for (Map.Entry<String, String> fieldValues : metadata.entrySet()) {
                ...
                String value = fieldValues.getValue();
                CategoryMetadataField categoryMetadataField = metadataFieldRepository.findByName(field);
                CategoryMetadataFieldValues categoryMetadataFieldValues = metadataFieldValuesRepository.findMetadataFieldValue(categoryMetadataField.getId(), productVariation.getProduct().getProductcategory().getPcId());
                List<String> list = Arrays.asList(categoryMetadataFieldValues.getMetadataValues().split(","));
