...
public class CustomerController {
    ...
    public String addCustomer(@Valid @RequestBody Customer user,@RequestHeader(name = "Accept-Language", required = false) Locale locale) {
    ...
    public String activateCustomer(@PathVariable String token,@RequestHeader(name = "Accept-Language", required = false) Locale locale){
        String message = customerService.activateCustomer(token,locale);
        return message;
    }

    @GetMapping("/re-sent-link/{email}")
    public String reSentLink(@PathVariable String email,@RequestHeader(name = "Accept-Language", required = false) Locale locale) {
        String message = customerService.reSentLink(email,locale);
        return message;
    }

    @PutMapping("/customer/update/profile")
    public String editCustomer(@RequestBody CustomerDto customer,@RequestHeader(name = "Accept-Language", required = false) Locale locale){
        customerService.editCustomer(customer);
        return messageSource.getMessage("customer.update.message", null, locale);
    }

    @PutMapping("customer/update/password")
    public String updatePassword(@RequestBody PasswordDto passwordDto,@RequestHeader(name = "Accept-Language", required = false) Locale locale){
